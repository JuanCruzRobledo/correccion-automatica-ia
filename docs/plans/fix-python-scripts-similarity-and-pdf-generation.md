...
|--------|--------------|---------|------------|
| Python no disponible en producción | **ALTA** | **CRÍTICO** | Usar Docker con Node+Python o reescribir en Node.js |
...
| Deploy en Vercel/Netlify imposible | Alta | Alto | Usar Railway/Render/VPS con Docker |
...
## ⚠️ CONSIDERACIONES CRÍTICAS PARA DEPLOY EN PRODUCCIÓN

### Problema 1: Python No Estará Disponible en Todos los Entornos

**Escenarios de Deploy**:

#### A) Railway / Render / Heroku (PaaS)
- **Problema**: Buildpack por defecto es Node.js, no incluye Python
- **Solución**: Usar multi-buildpack o imagen Docker con Node + Python

#### B) Vercel / Netlify (Serverless)
- **Problema**: Funciones serverless tienen límites de tiempo (10-60 seg)
- **Problema**: No se puede ejecutar procesos externos como Python
- **Solución**: **NO COMPATIBLE** - Migrar a opción A o C

#### C) VPS / VM (DigitalOcean, AWS EC2, etc)
- **Problema**: Python debe instalarse manualmente
- **Solución**: Script de instalación o Dockerfile

### Problema 2: N8N Debe Estar Corriendo

**Opciones**:
1. **N8N en el mismo servidor** (VM/Docker Compose)
2. **N8N Cloud** (n8n.cloud - pago)
3. **N8N en servidor separado** (VPS dedicado)

### Problema 3: Instalación de Dependencias Python

**En desarrollo**:
```bash
pip install reportlab requests pymongo
```

**En producción** (Railway/Render):
- Crear archivo `requirements.txt` en la raíz del proyecto
- El buildpack de Python lo detectará automáticamente

**En Docker**:
- Instalar Python y pip en el Dockerfile
- Copiar requirements.txt y ejecutar pip install

---

## 🐳 Solución Recomendada: Docker Multi-Stage

### Dockerfile Propuesto

```dockerfile
# Stage 1: Base con Node.js y Python
...
# Crear symlink para que 'python' apunte a 'python3'
...
# Copiar requirements.txt de Python
COPY backend/scripts/python/requirements.txt ./backend/scripts/python/

# Instalar dependencias de Python
RUN pip3 install -r backend/scripts/python/requirements.txt
...
### docker-compose.yml Completo (Backend + N8N)

```yaml
version: '3.8'

services:
  # Backend Node.js con Python
  backend:
    build:
      context: .
...
### Pre-Deploy
- [ ] Crear `requirements.txt` con dependencias Python en raíz del proyecto
- [ ] Probar Dockerfile localmente: `docker build -t backend .`
- [ ] Probar docker-compose localmente: `docker-compose up`
- [ ] Verificar que Python funciona: `docker exec -it <container> python --version`
- [ ] Verificar que N8N es accesible desde backend

### Deploy en Railway/Render (Con Buildpacks)
- [ ] Agregar buildpack de Python:
  - Railway: Detecta automáticamente por `requirements.txt`
  - Render: Configurar en `render.yaml` o settings
- [ ] Variables de entorno:
  - `N8N_WEBHOOK_GET_CORRECTIONS`
  - `N8N_BASE_URL`
- [ ] Verificar logs que Python se instaló correctamente

### Deploy con Docker
- [ ] Subir imagen a Docker Hub o Registry
- [ ] Configurar docker-compose en servidor
- [ ] Asegurar volúmenes persistentes para N8N
- [ ] Configurar red interna para comunicación Backend ↔ N8N

### Post-Deploy
- [ ] Importar workflows de N8N desde `n8n-workflows/*.json`
- [ ] Configurar credenciales de Google en N8N
- [ ] Probar endpoint de generación de PDFs
- [ ] Verificar logs de Python en producción

---

## 🔧 Alternativa: Reescribir en Node.js (Si el Deploy es Problemático)

Si ejecutar Python en producción resulta muy complejo, considerar:

### Opción B: Reescribir Scripts en Node.js

**Pros**:
- Sin dependencias externas
- Compatible con cualquier entorno de deploy
- Más fácil de debuggear

**Contras**:
- Mayor tiempo de desarrollo
- Riesgo de bugs al reescribir lógica
- Perder fidelidad 100% con scripts originales

**Librerías Node.js equivalentes**:
- ReportLab → `pdfkit` o `pdf-lib`
- Requests → `axios` (ya instalado)

**Estimación**: +10-15 horas de desarrollo adicional
...
- **N8N** corriendo y accesible desde el backend
